...
// This script generates a release note using a template made in go template format
// It uses flags to pass data to the template
// Each flag has a corresponding template action of the form {{.flag}}
// The template to be used is passed from stdin
// ex: -changelog flag is represented as {{.changelog}} in the template
// Available flags:
// -changelog 	(string) path to the changelog file
// -title 		(string)
// -summary		(string)
// -deb-version	(string)
// -contributors (string)
...
var (
	...
	title        = flag.String("title", "{{.title}}", "release title")
